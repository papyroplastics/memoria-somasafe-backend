...
shared schema (`shared/dataset.proto`). Its Python stub is generated by the shared
module's own makefile, into `shared/gen/code/`, and imported from there as
`shared.gen.code.dataset_pb2` — the firmware harness reads the same exports, so the
bindings live where both modules can reach them rather than inside either one:

```bash
make shared   # links ../shared, then protoc shared/dataset.proto -> shared/gen/code/
```
...
what used to be a separate non-normalizing `infer` graph for the int8 export is now just
`eval`, since the input it receives is already normalized either way.
...
params come from its whole clean recording, so the same subject normalizes identically
whichever dataset asked, and the two `--dataset` runs stay comparable. A device sees its
wearer's whole day, not a filtered slice of it.

### What is on disk, and what is not

Only two stages are written out, both by `scripts/system/get_dataset.py`:

| directory | what it holds |
| --- | --- |
| `clean-signals/S*/` | raw BVP (64 Hz), ACC magnitude (32 Hz), activity track on the BVP grid |
...
of it for the consumers that cannot import the backend — the Android app and the firmware
harness both read that one file.
...
All five kinds go into the mix, drawn at random per span. The per-kind
`anomalous-signals/<kind>/` sets are the same anomalies applied to *every* window instead,
which is what lets `scripts/figures/anomaly_kinds.py` weigh each kind on its own rather
than through an aggregate the others dominate.
...
`calibrate_fpr`, `anomaly_detection`, `anomaly_kinds`, `subject_roc`, `plot_signals` and
...
The case studies live in four `scripts/figures/` scripts:
...
- **`anomaly_kinds.py`** breaks the aggregate down by anomaly kind. At one fixed operating
  point it scores each kind on that kind's own fully-anomalous copy of the held-out
  subjects, and reports both the recall a deployed detector would get and the
  threshold-free AUC against the same subjects' clean windows. The AUC is the load-bearing
  number: below 0.5 the score is *inverted* for that kind — the detector ranks those
  windows as more normal than clean signal — which no choice of threshold can fix and
  which a mixed-set metric silently averages away. That is what bradycardia does to a
  waveform autoencoder, and it is the single biggest drag on `cnn-ae`'s ROC. Writes
  `anomaly_kinds.png` + `.csv`/`.yaml` to `results/<model>/anomaly_kinds/<dataset>/`.
...
uv run -m scripts.figures.anomaly_kinds cnn-ae                          # per-kind detectability, every kind incl. the excluded ones
...
### Export a subject for the app and the firmware harness

`export_subject_data.py` packs one subject's windows into the `.ssds` protobuf, written to
...
streams the windows' raw samples back out as a sensor would, and `test_model.py` checks the
device's own features and score against the ones stored beside them — so it needs a gapless
export, i.e. no `--missing-*`.
...
uv run -m scripts.system.export_subject_data 1 --clean                     # clean (anomaly-free) signals, every window normal
```

`--clean` exports the anomaly-free clean signal instead of the anomaly mix; every window
is then normal (
...
3. The client polls `GET /model/quantize/result/{job_id}`. The endpoint authorizes and
   ...
   the `X-Model-Signature` / `X-Contract-Version` headers once `done` — the
