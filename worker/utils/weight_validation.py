...
def validate_submission(submission: WeightSubmission, expected_param_count: int,
    ...
    """Reason the submission is unusable, or None if it is valid. The MSE gate
    compares against ``reference``'s parameters using the threshold computed by
    the aggregation round that produced it; without a reference or a threshold
    (no aggregation has happened yet) that check is skipped."""
