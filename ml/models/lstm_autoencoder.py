...
class LSTMAutoencoder(TrainableAutoencoder):
    ...
    The encoder's final state is linearly projected to the latent, which seeds the
    decoder's initial state; a fixed sinusoidal positional encoding drives each
    decoder step (content-independent, so it can't collapse to a flat line nor leak
    signal into the score). The reduced-length output is upsampled and smoothed back
    to the full window. Encoder sees ``[BVP, ACC]``; decoder reconstructs BVP only."""
    ...
    def __init__(self, name: str, batch_size: int, seq_len: int, n_signals: int,
        ...
        # Encoder: 3 stride-2 convs (downsample x8) -> LSTM -> linear latent.
